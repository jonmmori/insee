...
int main(int argc, char *argv[])
{
	...
	long servers, switches, switches_aux, ports_network, total_ports, ports_switches, ports_servers, param_nic;
	char filename_params[100];

	if (argc < 6) {
		printf("5 parameters are needed for M_GRAPH <n, k, r, nnic, seed>.\n");
		exit(-1);
	}
	param_n = atoi(argv[1]);
	param_k = atoi(argv[2]);
	param_r = atoi(argv[3]);
	param_nic = atoi(argv[4]);
	param_seed = atoi(argv[5]);

	if (param_r < 3) {
		printf("The number of ports must be higher than 2.\n");
		exit(-1);
	}
	servers = ((param_n * (param_k - param_r)) / param_nic);
	switches = param_n;
	...
	switches_aux = switches / param_nic;
	random_graph =  malloc(sizeof(graph_t*) * param_nic);
	for(k = 0; k < param_nic; k++){
		random_graph[k] = malloc(sizeof(graph_t) * switches_aux);
		for(i = 0; i < switches_aux; i++) {
			random_graph[k][i].nedges = ports_switches;
			random_graph[k][i].fedges = random_graph[k][i].nedges;
			random_graph[k][i].active = 1;
			random_graph[k][i].edge = malloc(sizeof(edge_t) * random_graph[k][i].nedges);
			for(j = 0; j < random_graph[k][i].nedges; j++) {
				random_graph[k][i].edge[j].neighbour.node = -1;
				random_graph[k][i].edge[j].neighbour.edge = -1;
				random_graph[k][i].edge[j].active = 1;
			}
		}
		generate_rrg(random_graph[k], switches_aux, ports_switches, param_seed + k);
	}
	//export_graph_edges(random_graph,switches, filename_params);
	//export_graph_adjacency(random_graph,switches);
	//export_graph_rrg_insee(random_graph,switches, servers, param_nic, ports_servers, filename_params);
	export_graph_rrg_socnetv(random_graph,switches, servers, param_nic, ports_servers, filename_params);
	finish_topo_jellyfish(switches, param_nic);
	return(1);
}
 ...
 */
void finish_topo_jellyfish(long switches, long param_nic)
{

	long i, j, switches_aux;

	switches_aux = switches / param_nic;

	for(j = 0; j < param_nic; j++){
		for(i = 0; i < switches_aux; i++) {
			free(random_graph[j][i].edge);
		}
		free(random_graph[j]);
	}
	free(random_graph);
 ...
 */
/*
   void export_graph_rrg_insee(graph_t **rg, long switches, long servers, long param_nic, long ports_servers, char *topo_name)
   {
   ...
   for(i = 0; i < servers; i++) {
   for(k = 0; k < param_nic; k++) {
   printf("(%ld,%ld) ", ((i / ports_servers) * param_nic) + servers + k, i % ports_servers); 
   }
   printf("\n");
   }

   for(i = 0; i < switches; i++) {
   for(j = 0; j < ports_servers; j++){
   printf("(%ld,%ld) ", ((switches - ((param_nic * servers) / ports_servers)) + ((i / param_nic) * ports_servers) + j), i % param_nic);
   }

   for(k = 0; k < rg[i].nedges; k++) {
   printf("(%ld,%ld) ", rg[i].edge[k].neighbour.node + servers, rg[i].edge[k].neighbour.edge + ports_servers);
   }
   printf("\n");
   }
   }
 ...
 * Print the network structure (without node) as a an adjacency matrix. Just for testing purposes.
 */
void export_graph_rrg_socnetv(graph_t **rg, long switches, long servers, long param_nic, long ports_servers, char *topo_name)
{

	long i, j, k, h, switches_aux;
	...
	for(i = 0; i < servers; i++) {
		...
		for(k = 0; k < param_nic; k++) {
			//printf("(%ld,%ld) ", ((i / ports_servers) * param_nic) + servers + k, i % ports_servers); 
			printf("%ld ", ((i / ports_servers) * param_nic) + servers + k + 1);
		}
		printf("\n");
	}

	for(i = 0; i < switches_aux; i++) {
		for(h = 0; h < param_nic; h++){
			printf("%ld ", (param_nic * i) + h + servers + 1);
			for(j = 0; j < ports_servers; j++){
				//printf("(%ld,%ld) ", ((switches - ((param_nic * servers) / ports_servers)) + ((i / param_nic) * ports_servers) + j), i % param_nic);
				printf("%ld ", ((switches - ((param_nic * servers) / ports_servers)) + ((((i * param_nic) + h) / param_nic) * ports_servers) + j + 1));
			}

			for(k = 0; k < rg[h][i].nedges; k++) {
				//printf("(%ld,%ld) ", rg[i].edge[k].neighbour.node + servers, rg[i].edge[k].neighbour.edge + ports_servers);
				if(rg[h][i].edge[k].neighbour.node != -1)
					printf("%ld ", (rg[h][i].edge[k].neighbour.node * param_nic) + h + 1 + servers);
			}
			printf("\n");
		}
	}
