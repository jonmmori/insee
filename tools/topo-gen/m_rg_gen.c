...
int main(int argc, char *argv[])
{
    ...
    long servers, switches, ports_network, total_ports, ports_switches, ports_servers, param_nic;
...
void export_graph_rrg_socnetv(graph_t *rg, long switches, long servers, long param_nic, long ports_servers, char *topo_name)
{
    ...
    for(i = 0; i < switches; i++) {
	    ...
	    for(j = 0; j < ports_servers; j++){
		//printf("(%ld,%ld) ", ((switches - ((param_nic * servers) / ports_servers)) + ((i / param_nic) * ports_servers) + j), i % param_nic);
		printf("%ld ", ((switches - ((param_nic * servers) / ports_servers)) + ((i / param_nic) * ports_servers) + j + 1));
	    }

            for(k = 0; k < rg[i].nedges; k++) {
                //printf("(%ld,%ld) ", rg[i].edge[k].neighbour.node + servers, rg[i].edge[k].neighbour.edge + ports_servers);
		printf("%ld ", rg[i].edge[k].neighbour.node + servers + 1);
            }
            printf("\n");
    }
}
