...
*/
void alltoall(long nodes, long size)
{
	long i, j;

	for (i=0; i<nodes; i++)
		for (j=1; j<nodes; j++)
			insert_send(i,  (i+j)%nodes,   i,      size);

	for (i=0; i<nodes; i++)
		for (j=1; j<nodes; j++)
			insert_recv((i+j)%nodes,  i,   i,      size);
}
...
*@param size    The size of the burst.
*/
void alltoall2(long nodes, long size)
{
        long i, j;
        ...
        for (i=0; i<nodes; i++)
                for (j=0; j<nodes; j++)
			if (i!=j)
                        	insert_send(i,  j,   i,      size);

        for (i=0; i<nodes; i++)
                for (j=0; j<nodes; j++)
			if (i!=j)
                        	insert_recv(j,  i,   i,      size);
}
...
int main(int argc, char *argv[])
{
	...
	if (DIMEMAS_HEADER_NEEDED != 0){
		printf("#DIMEMAS:_alltoall%d_%d:0:%d(1",nodes,size,nodes);
		for (i=1; i<nodes; i++)
			printf(",1");
		printf("),0\n");
	}
