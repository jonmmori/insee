...
void frame_packetizer(struct packet * pack, unsigned long FSIN_destination){
	...
	pack->FSIN_packets = (struct FSIN_packet *)alloc (sizeof(struct FSIN_packet) * pack->num_FSIN_packets);
	for (i = 0; i < pack->num_FSIN_packets; i++){
	...
	tv.tv_sec = TV_SEC;
  	...
  	while (1) {
		...
		if (FD_ISSET(sock, &lb)) {//nueva conexion Simics
			...
			aux = (struct arp_record *)alloc (sizeof(struct arp_record));
			...
			for (escritor=0; escritor<FD_SETSIZE; escritor++) {
				if(FD_ISSET(escritor, &lb)) {
					...
					leidos = 0;
					buf = (char *) alloc ((count + sizeof(count) + sizeof(when)) * sizeof(char));
					...
					pack = (struct packet *)alloc (sizeof(struct packet));
					pack->buffer = buf;
					pack->length = count;
...
new_sync = (struct sync *)alloc (sizeof(struct sync));
